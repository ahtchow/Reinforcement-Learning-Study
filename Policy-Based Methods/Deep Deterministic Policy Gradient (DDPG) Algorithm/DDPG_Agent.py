...
class DDPG_Agent():
    ...
    def act(self, state, add_noise=True):
        ...
        self.actor_local.eval() # Evaluation Mode
        with torch.no_grad():
            # Determine action using Actor
            action = self.actor_local(state).cpu().data.numpy()
        self.actor_local.train() # Back to train mode

        if add_noise:
            action += self.noise.sample()
        # Limit continious range from (-1,1)
        return np.clip(action, -1, 1)


    def reset(self):
        """Reset the noise for OU Noise"""
        self.noise.reset()
    ...
    def learn(self, experiences, gamma):
        ...
        q_prime = self.critic_target(next_states, actions_next)
